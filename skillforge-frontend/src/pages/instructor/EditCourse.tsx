...
const EditCourse: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadCourse = async () => {
            try {
                setLoading(true);
                const overview = await instructorAPI.getCourseStructure(id);
                setTitle(overview.course.title);
                setDescription(overview.course.description);
                setSubject(overview.course.subject);
                setDifficultyLevel(overview.course.difficultyLevel);
                setPrerequisites(overview.course.prerequisites ? overview.course.prerequisites.split('||').filter(Boolean) : []);
                setLearningOutcomes(overview.course.learningOutcomes ? overview.course.learningOutcomes.split('||').filter(Boolean) : []);
                setModules(overview.modules.map((module) => ({
                    ...
                    contents: module.contents.map((content) => ({
                        ...
                        title: content.title,
                        contentType: content.contentType === 'MODULE' ? 'TEXT' : content.contentType,
                ...
                setError(loadError.response?.data?.message || 'Failed to load course structure.');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSave = async () => {
        ...
        try {
            setSaving(true);
            setError(null);
            await instructorAPI.updateCourse(id, {
                title,
                description,
                subject,
                difficultyLevel,
            ...
            setError(saveError.response?.data?.message || 'Failed to update course.');
        } finally {
            setSaving(false);
        }
    };
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 2, mb: 4 }}>
                <Box>
                    ...
                    <Typography color="text.secondary">
                        Refine the module tree, nested content, and exam-ready learning flow for this course.
                    </Typography>
                </Box>

                <Button variant="outlined" startIcon={<AutoAwesome />} onClick={handleAiSuggestions} disabled={aiLoading}>
                    {aiLoading ? 'Generating...' : 'AI Refresh Modules'}
                </Button>
            </Box>
                    ...
                    <Card sx={{ borderRadius: 4 }}>
                        <CardContent sx={{ p: 3 }}>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                                ...
                                <Button startIcon={<Add />} variant="contained" onClick={() => setModules((current) => [...current, { title: '', description: '', durationMinutes: 45, metadata: '', contents: [] }])}>
                                    Add Module
                                </Button>
                            </Box>
                                        ...
                                        <CardContent sx={{ p: 2 }}>
                                            <Stack spacing={2}>
                                                ...
                                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                    ...
                                                    <Button size="small" startIcon={<Add />} onClick={() => setModules((current) => current.map((item, moduleIndex) => moduleIndex === index ? {
                                                        ...item,
                                                        contents: [...item.contents, { title: '', contentType: 'TEXT', contentText: '', contentUrl: '', durationMinutes: 15, metadata: '' }],
                                                    } : item))}>
                                                        Add Content
                                                    </Button>
                                                </Box>
                    ...
                    <Card sx={{ borderRadius: 4, position: 'sticky', top: 88 }}>
                        <CardContent sx={{ p: 3 }}>
                            ...
                            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                                Learners will immediately see updated modules and assessments in their enrolled course flow.
                            </Typography>
                            <Button variant="contained" fullWidth size="large" startIcon={<Save />} onClick={handleSave} disabled={saving} sx={{ py: 1.5, textTransform: 'none', fontWeight: 800 }}>
